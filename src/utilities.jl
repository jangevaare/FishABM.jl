  ...
  for r in 1:size(environment_assumptions.id)[1]
    for c in 1:size(environment_assumptions.id)[2]
      if 0 < r-1 <= size(environment_assumptions.id)[1]
        if 0 < c-1 <= size(environment_assumptions.id)[2] && isnan(environment_assumptions.id[r-1,c-1]) == false
          movement[environment_assumptions.id[r,c], environment_assumptions.id[r-1,c-1]] = weights[1,1]
        end
        if 0 < c <= size(environment_assumptions.id)[2] && isnan(environment_assumptions.id[r-1,c]) == false
      ...
      if 0 < r <= size(environment_assumptions.id)[1]
        if 0 < c-1 < size(environment_assumptions.id)[2] && isnan(environment_assumptions.id[r,c-1]) == false
          movement[environment_assumptions.id[r,c], environment_assumptions.id[r,c-1]] = weights[2,1]
        end
        if 0 < c <= size(environment_assumptions.id)[2] && isnan(environment_assumptions.id[r,c]) == false
