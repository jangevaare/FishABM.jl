...
function kill!(agent_db::DataFrame, environment_assumptions::environment_assumptions, agent_assumptions::agent_assumptions, cohort::Int64, week::Int64)
  ...
  for i = 1:length(agent_db[cohort, week][:alive])
    killed = minimum([rand(Poisson(agent_db[cohort, week][:alive][i]*agent_assumptions.mortality_natural[environment_assumptions.habitat[agent_db[cohort, week][:location][i].==environment_assumptions.id],agent_db[cohort, week][:stage][i]])), agent_db[cohort, week][:alive][i]])
    ...
    if environment_assumptions.risk[agent_db[cohort, week][:location][i].==environment_assumptions.id]
      killed = minimum([rand(Poisson(agent_db[cohort, week][:alive][i]*agent_assumptions.mortality_risk[agent_db[cohort, week][:stage][i]])), agent_db[cohort, week][:alive][i]])
