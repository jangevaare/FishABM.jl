...
function spawn!(agent_db::DataFrame, stock_db::stock_db, stock_assumptions::stock_assumptions, environment_assumptions::environment_assumptions, cohort::Int)
  ...
  brood_size = rand(Poisson(compensation_factor_a*stock_assumptions.mean_brood_size[1]), rand(Binomial(stock_db.population[end,1], cdf(Geometric(1/max(compensation_factor_b*stock_assumptions.age_at_half_mature-3,1)),0)*0.5)))
