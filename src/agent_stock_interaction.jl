...
function Spawn!(agent_db::DataFrame, StockDB::StockDB, StockAssumptions::StockAssumptions, EnvironmentAssumptions::EnvironmentAssumptions, cohort::Int)
  ...
  """
  if isnan(StockAssumptions.fecundity_compensation)
    compensation_factor_a = 1
  else
    compensation_factor_a = 2*(1-cdf(Normal(StockAssumptions.carrying_capacity, StockAssumptions.carrying_capacity/StockAssumptions.fecundity_compensation), sum(StockDB.population[end,:][1,])))
  end
  @assert(0.01 < compensation_factor_a < 1.99, "Population regulation has failed, respecify simulation parameters")
  if isnan(StockAssumptions.maturity_compensation)
    compensation_factor_b = 1
  else
    compensation_factor_b = 2*(cdf(Normal(StockAssumptions.carrying_capacity, StockAssumptions.carrying_capacity/StockAssumptions.maturity_compensation), sum(StockDB.population[end,:][1,])))
  end
  ...
  brood_location = sample(EnvironmentAssumptions.id[EnvironmentAssumptions.spawning], length(brood_size))
